import Foundation
import Nimble
import Quick
import CryptoStarterPack
import Bedrock
import AwesomeDictionary
@testable import MerkleModels

final class MerkleModelsSpec: QuickSpec {
	override func spec() {
		let key = UInt256.random()
		let publicKeyDigest = BaseCrypto.hash(publicKey.toBoolArray())!
		let encryptedAddress = EncryptedAddress256(addressHash: UInt256(0), symmetricKey: key, recipientPublicKeys: [publicKey])
		describe("Encryption") {
			it("encrypted data should decrypt to original") {
				expect(encryptedAddress).toNot(beNil())
				let decrypted = encryptedAddress!.decryptKey(publicKeyDigest: UInt256(raw: publicKeyDigest)!, privateKey: privateKey)
				expect(decrypted).toNot(beNil())
				expect(decrypted!).to(equal(key))
			}
		}
		describe("Signature") {
			let signedMessage = SignedMessage256(privateKey: privateKey, publicKey: publicKey, message: encryptedAddress!)
			it("should verify successfully") {
				expect(signedMessage).toNot(beNil())
				expect(signedMessage!.verify(publicKey: publicKey)).to(beTrue())
			}
		}
	}
}

let privateKey = """
-----BEGIN RSA PRIVATE KEY-----

Xj9uKUw0ExH06Qa5GGAcVhcw0PyGtM09Mu93iQbMUukBqQGQN5Z3
-----END RSA PRIVATE KEY-----
"""

let publicKey = """
-----BEGIN PUBLIC KEY-----

XwIDAQAB
-----END PUBLIC KEY-----
"""
